				...
				<main>
					<div class="row contacts">
						<div class="col invoice-to">
							...
							<h2 class="to">{{ getUserNameById($reservation->user->id) }}</h2>
							<div class="address">{{ getUserById($reservation->user->id)->telephone }}</div>
							<div class="email"><a href="mailto:{{ getVoyageurById($reservation->user->id)->email }}">{{ getVoyageurById($reservation->user->id)->email }}</a></div>
						</div>
						<div class="col invoice-details">
							...
							if ($reservation->type_destination == 2) {
								...
								<div class="date">
									Compagnie: 
									<b>
										<?= getUserNameById($reservation->destinationCompagnie->user_id)?>
									</b>
								</div>
								<?php	 	 	
							}
							...
							<div class="date">
								...
								<b>
									...
									if ($reservation->type_destination == 1) {
										...
										echo getFullDate($reservation->date_depart) . " à " .$reservation->destinationCompagnie->heure;
									}
					...
					<div class="row">
						<div class="col-lg-8">
							...
							if ($reservation->type_destination == 1) {
								...
								<p>
									Lieu de Départ  : <b>
										{{ $reservation->destination->ville_demarrage }}, {{ $reservation->destination->pays_demarrage }} 
									</b><br>
									Lieu d'Arrivée :  <b>
								...
								?>
								<p>
									Lieu de Départ : <b>{{ $reservation->destinationCompagnie->ville_demarrage }}, {{ $reservation->destinationCompagnie->pays_demarrage }}</b><br>
									Lieu d'Arrivée : <b>
										@if($reservation->ligne_destination_id == 0)
										{{ $reservation->destinationCompagnie->ville_destination }}, {{ $reservation->destinationCompagnie->pays_destination }}
										@else
